...
@section('content')
    ...
    <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
        <ol class="breadcrumb">
            <h5 class="title">New Form Setup</h5>
            <li class="breadcrumb-item"><a href="{{ route( auth()->user()->getRoles()[0]. '.appraisals.form-setups.index') }}">Form Setups</a></li>
            <li class="breadcrumb-item active" aria-current="page">New Form Setup</li>
        </ol>
    </nav>
    <div class="card card-dashboard">
        <div class="card-body">
            <form method="post" action="{{  route( auth()->user()->getRoles()[0]. '.appraisals.form-setups.store') }}">
                @csrf
                <div class="row">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="batch_setup_id" class="form-select @error('batch_setup_id') is-invalid @enderror">
                                ...
                                @foreach ($batchSetups as $batchSetup)
                                    <option value="{{ $batchSetup->id }}">{{ $batchSetup->code }}</option>
                                @endforeach
                            </select>
